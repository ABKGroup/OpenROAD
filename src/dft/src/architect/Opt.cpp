    ...
    const std::function<int64_t(std::size_t, std::size_t)>& cost_fn)
{
  ...
  while (order.size() < n) {
    ...
    for (std::size_t i = 0; i < n; ++i) {
      ...
      const int64_t cost = cost_fn(cur, i);
      if (!found || cost < best_cost
          || (cost == best_cost && names[i] < names[best])) {
        found = true;
        best = i;
    ...
    if (!found) {
      logger->error(utl::DFT, 92, "Couldn't find next node to order");
    }
    used[best] = true;
    order.push_back(best);
    cur = best;
  ...
  for (int pass = 0; pass < 2; ++pass) {
    ...
    for (std::size_t pos = 1; pos + 1 < order.size(); ++pos) {
      ...
      const int64_t before = cost_fn(prev, a) + cost_fn(a, b)
                             + (has_next ? cost_fn(b, next) : 0);
      const int64_t after = cost_fn(prev, b) + cost_fn(b, a)
                            + (has_next ? cost_fn(a, next) : 0);
      if (after < before) {
        std::swap(order[pos], order[pos + 1]);
        improved = true;
      }
    ...
    const std::function<int64_t(std::size_t, std::size_t)>& edge_cost)
{
  ...
  while (idx < components.size()) {
    ...
    std::size_t end = idx;
    while (end < components.size() && components[end].priority == pr) {
      ++end;
    }

    // Build internal orders for components in this priority bucket.
    for (std::size_t ci = idx; ci < end; ++ci) {
        ...
        if (succ_it != succ.end() && succ_it->second != to) {
          if (logger) {
            logger->warn(
                utl::DFT,
                94,
                "Scan constraints: fixed-edge conflict on '{}' "
                "(multiple successors); ignoring extra edge to '{}'.",
                names[from],
                names[to]);
          }
          continue;
        ...
        if (pred_it != pred.end() && pred_it->second != from) {
          if (logger) {
            logger->warn(
                ...
                95,
                "Scan constraints: fixed-edge conflict on '{}' "
                "(multiple predecessors); ignoring extra edge from "
                "'{}'.",
                names[to],
                names[from]);
          }
    ...
    // Order components within this priority bucket.
    const std::size_t cn = end - idx;
    if (cn == 1) {
    ...
    const std::vector<std::size_t> comp_order
        = orderNodesByCost(cn, start_comp, comp_names, config, logger, comp_cost);
                                    ...
                                    utl::Logger* logger)
{
  ...
  const auto edge_cost = [&](std::size_t src, std::size_t dst) -> int64_t {
    ...
    return scaleEdgeCost(manhattanDist(origins[src], origins[dst], vertical_weight),
                         timing_mul[src]);
  };

  if (hasScanOrderConstraints(config)) {
    optimizeScanWirelengthWithConstraints(
        cells, config, origins, names, logger, edge_cost);
    return;
  }

  if (config.getScanOrderSolver()
      == ScanArchitectConfig::ScanOrderSolver::ScanOpt) {
    if (n > kScanOptMaxMatrixCells) {
      logger->warn(
          utl::DFT,
          72,
                            ...
                            utl::Logger* logger)
{
  ...
  const auto edge_cost = [&](std::size_t src, std::size_t dst) -> int64_t {
    return scaleEdgeCost(manhattan(origins[src], origins[dst]), timing_mul[src]);
  };
  ...
  // edges from timing-critical sources). Use a directed greedy heuristic with a
  // small local-improvement pass rather than symmetric 2-opt/farthest-insertion
  // moves.
