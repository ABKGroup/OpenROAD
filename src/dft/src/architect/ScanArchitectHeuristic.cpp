    ...
    utl::Logger* logger)
{
  ...
  for (auto& uptr : domain_cells) {
    ...
    PlacedScanCell placed;
    placed.cell = std::move(uptr);
    placed.origin = raw->getOrigin();
    placed.bits = bits;
    placed.name = raw->getName();
  ...
  if (max_cell_bits > max_length && logger) {
    logger->warn(utl::DFT,
                 69,
  ...
  if (total_bits > chain_count * max_length && logger) {
    logger->warn(utl::DFT,
                 70,
  ...
  while (seed_indices.size() < k) {
    ...
    for (std::size_t i = 0; i < n; ++i) {
      const bool is_seed
          = std::find(seed_indices.begin(), seed_indices.end(), i)
            != seed_indices.end();
      if (is_seed) {
        continue;
      }
...
{
  // For each hash_domain, distribute scan cells over the scan chains.
